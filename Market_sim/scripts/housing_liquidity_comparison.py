...
if _BAYESIAN_NPZ.exists():
    import numpy as _np
    P_TOKENIZED = _np.load(_BAYESIAN_NPZ)["P_mean"].tolist()
    print(f"  [housing_sim] P_TOKENIZED loaded from Bayesian posterior: {_BAYESIAN_NPZ.name}")
else:
    P_TOKENIZED = _P_TOKENIZED_FALLBACK
    print(f"  [housing_sim] P_TOKENIZED using hand-crafted fallback (run bayesian_cre_transition.py to update)")
...
def run_simulation(
    ...
    return months, fund_prices, regimes
