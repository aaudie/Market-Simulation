...
def combine_multi_reit_data(
    ...
    if method == "longest":
        ...
        # Filter out any series with NaN values
        clean_series = []
        for symbol, prices in reit_data.items():
            prices = np.asarray(prices, dtype=np.float64)
            if np.any(np.isnan(prices)):
                print(f"  Warning: {symbol} contains NaN values, excluding from training")
                continue
            clean_series.append(prices)
        
        if not clean_series:
            raise ValueError("All REIT series contain NaN values!")
...
def prepare_dmm_training_data(
    ...
    for i in range(0, len(traditional_data) - window_size, stride):
        window = traditional_data[i:i + window_size]
        # Validate window: must be exactly window_size and contain no NaNs
        if len(window) == window_size and not np.any(np.isnan(window)):
            trad_windows.append(window)
    ...
    if isinstance(tokenized_data, list):
        ...
        for reit_idx, reit_prices in enumerate(tokenized_data):
            # Ensure prices are numpy array and have no NaNs
            reit_prices = np.asarray(reit_prices, dtype=np.float64)
            if np.any(np.isnan(reit_prices)):
                print(f"    Warning: REIT {reit_idx} contains NaN values, skipping...")
                continue
            
            for i in range(0, len(reit_prices) - window_size, stride):
                window = reit_prices[i:i + window_size]
                # Double-check: must be exactly window_size and contain no NaNs
                if len(window) == window_size and not np.any(np.isnan(window)):
                    token_windows.append(window)
    else:
        # Average/median/longest method: single array
        tokenized_data = np.asarray(tokenized_data, dtype=np.float64)
        if np.any(np.isnan(tokenized_data)):
            print(f"  Warning: Tokenized data contains NaN values")
            # Remove NaN-containing sections
            valid_indices = ~np.isnan(tokenized_data)
            tokenized_data = tokenized_data[valid_indices]
        
        for i in range(0, len(tokenized_data) - window_size, stride):
            window = tokenized_data[i:i + window_size]
            if len(window) == window_size and not np.any(np.isnan(window)):
                token_windows.append(window)
    ...
    # Convert to array with explicit shape validation
    prices_array = np.array(all_windows, dtype=np.float64)
    
    # Final validation
    if prices_array.shape[1] != window_size:
        raise ValueError(f"Window size mismatch: expected {window_size}, got {prices_array.shape[1]}")
    if np.any(np.isnan(prices_array)):
        nan_count = np.sum(np.isnan(prices_array))
        raise ValueError(f"Generated prices array contains {nan_count} NaN values!")
