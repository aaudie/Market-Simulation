...
# CRITICAL: Load .env file before importing qfclient
# The .env file is in Market_Sim/ directory (parent of Market_sim)
from dotenv import load_dotenv
env_path = parent_dir.parent / ".env"  # Market_Sim/.env
if env_path.exists():
...
def load_training_data_from_qfclient() -> Dict[str, np.ndarray]:
    ...
    if not QFCLIENT_AVAILABLE:
        ...
        if not reit_data:
            ...
            # Use "concatenate" to create separate training sequences from each REIT
            # This maximizes training data: 88 REITs × ~200 months = thousands of sequences
            # Alternative: "average" creates single smoothed series but loses data
            token_prices = combine_multi_reit_data(
                reit_data, 
                method="concatenate",  # Creates list of arrays, one per REIT
                min_length=60  # 5 years minimum (allows multiple windows)
            )
            
            if isinstance(token_prices, list):
                total_points = sum(len(p) for p in token_prices)
                print(f"  ✓ Created {len(token_prices)} separate REIT sequences ({total_points:,} total data points)")
            else:
